...
class PolicyGradient:
    ...
    def learn(self, epoch_i, entropy_weight, Ifprint=False):
        ...
        if not self.start_cpo:
            print("self.count",self.count, "np.mean(self.ep_ss)", np.mean(self.ep_ss), "0.5*self.safety_requirement", 0.5*self.safety_requirement, "self.start_cpo", self.start_cpo)
