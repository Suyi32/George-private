...
def train(params):
    ...
    while epoch_i < params['epochs']:
        ...
        tput = ((sim.predict(tput_state.reshape(-1, env.NUM_APPS))>0.8)* tput_state).sum() / NUM_CONTAINERS
...
def main():
    ...
    params['path'] = "secondobj_fpo_"+ str(hyper_parameter['alpha_choice']) + "_" + str(hyper_parameter['batch_C_numbers'])
