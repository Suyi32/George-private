...
class Network():
    ...
    def rl_sample(self, observation, source_batch_, index_data, RL_1_weights, RL_2_weights, RL_3_weights, env,
                  params, idx):
    ...
    def mega_sample(self, epoch_i, observation, source_batch_, index_data, RL_1_weights, RL_2_weights, RL_3_weights, env, params, idx):
        ...
        for i in range(mega_size):
            res.append(
                self.rl_sample(observation, source_batch_, index_data, RL_1_weights, RL_2_weights, RL_3_weights,
                               env, params, idx))
        return res
...
def train(params):
    ...
    while epoch_i < params['epochs']:
        ...
        batch_sample_results = ray.get(
            [sampler.mega_sample.remote(epoch_i, observation, source_batch_, index_data, RL_1_weights, RL_2_weights,
                                        RL_3_weights, env, params, idx=x) for x, sampler in enumerate(Samplers)]
        )
        ...
        for sample_idx in range(len(batch_sample_results)):
            ...
            RL_1.store_tput_per_episode(tput, (epoch_i * batch_size) + sample_idx, list_check, list_check_per_app,
                                        list_check_coex, list_check_sum)
            ...
            RL_1.store_training_samples_per_episode(batch_sample_results[sample_idx][1],
                                                    batch_sample_results[sample_idx][2], reward_episode_1,
                                                    safety_episode_1)
            RL_2.store_training_samples_per_episode(batch_sample_results[sample_idx][3],
                                                    batch_sample_results[sample_idx][4], reward_episode_2,
                                                    safety_episode_2)
            RL_3.store_training_samples_per_episode(batch_sample_results[sample_idx][5],
                                                    batch_sample_results[sample_idx][6], reward_episode_3,
                                                    safety_episode_3)
            ...
            if names['lowest_vio_' + str(tput_origimal_class)] > list_check:
                ...
                names['observation_optimal_1_vio_' + str(tput_origimal_class)].extend(
                    batch_sample_results[sample_idx][1])
                names['action_optimal_1_vio_' + str(tput_origimal_class)].extend(batch_sample_results[sample_idx][2])
                names['observation_optimal_2_vio_' + str(tput_origimal_class)].extend(
                    batch_sample_results[sample_idx][3])
                names['action_optimal_2_vio_' + str(tput_origimal_class)].extend(batch_sample_results[sample_idx][4])
                names['observation_optimal_3_vio_' + str(tput_origimal_class)].extend(
                    batch_sample_results[sample_idx][5])
            ...
            if list_check_ratio <= safety_requirement:
                if names['highest_tput_' + str(tput_origimal_class)] < tput:
                    ...
                    names['observation_optimal_1_' + str(tput_origimal_class)].extend(
                        batch_sample_results[sample_idx][1])
                    names['action_optimal_1_' + str(tput_origimal_class)].extend(batch_sample_results[sample_idx][2])
                    names['observation_optimal_2_' + str(tput_origimal_class)].extend(
                        batch_sample_results[sample_idx][3])
                    names['action_optimal_2_' + str(tput_origimal_class)].extend(batch_sample_results[sample_idx][4])
                    names['observation_optimal_3_' + str(tput_origimal_class)].extend(
                        batch_sample_results[sample_idx][5])
        ...
        if (epoch_i % 200 == 0) & (epoch_i > 1):
            ...
            np.savez(np_path, tputs=np.array(RL_1.tput_persisit), candidate=np.array(RL_1.episode),
                     vi_sum=np.array(RL_1.safe_persisit))
